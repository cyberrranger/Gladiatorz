...
$Queryzzz = mysql_query("SELECT DISTINCT a.id,a.forums_id,a.name,b.topics_id,a.important FROM ".TAB_TOPICS." AS a,".TAB_ANSWERS." AS b WHERE a.forums_id='".$Forum['id']."' AND b.topics_id=a.id ORDER BY b.id DESC,b.time DESC LIMIT $start, $eintraege_pro_seite");
while($Topic = mysql_fetch_array($Queryzzz))
{
  if($Topic[4] == 'j')
  {
	continue;
  ...
  $Query = mysql_query("SELECT * FROM ".TAB_TOPICS." WHERE id='".$Topic[0]."' LIMIT 1");
  ...
  <tr>
    ...
    <td class="forums" style="text-align:left;">&nbsp;'.$Important.'<a href="index.php?site=forum&showtopic='.$Topics['id'].'&seite='.$antw_seiten.'#unten" target="_self">'.$Topics['name'].' ('.$Posts.')</a></td>
